...
use meiki_application::{
    AnnotationDraftDto, ApplicationRuntime, ApplicationService, CheckAnswerRequest, Clock,
    ComparisonResultDto, CreateDeckRequest, DirectionDto, GradeDto, GradeReviewRequest, IdSource,
    ImportMediaRequest, MakeClozeRequest, MatchingPolicyDto, MediaAvailabilityDto, MediaRoleDto,
    SchedulingModeDto, StudyAvailabilityDto, TodayRequest, UndoReviewRequest,
...
#[test]
fn stale_review_fails_without_partial_state() {
    let fixture = authored_collection();
...
#[test]
fn released_schema_fixture_migrates_and_reopens_idempotently() {
    ...
    let restarted = service(&source_path, &clock, &ids);
    assert_eq!(
        restarted
            .prepare_study(&today("__all_decks__", NOW_MS))
            .expect("reopen the migrated fixture")
            .availability,
        StudyAvailabilityDto::EmptyCollection
    );
